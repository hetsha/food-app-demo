"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GooglePlacesController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const google_places_service_1 = require("./google-places.service");
const jwt_auth_guard_1 = require("../auth/guards/jwt-auth.guard");
let GooglePlacesController = class GooglePlacesController {
    constructor(googlePlacesService) {
        this.googlePlacesService = googlePlacesService;
    }
    async search(query) {
        return this.googlePlacesService.searchPlaces(query);
    }
    async reverseGeocode(lat, lng) {
        return this.googlePlacesService.reverseGeocode(parseFloat(lat), parseFloat(lng));
    }
};
exports.GooglePlacesController = GooglePlacesController;
__decorate([
    (0, common_1.Get)('search'),
    (0, swagger_1.ApiOperation)({ summary: 'Search places' }),
    __param(0, (0, common_1.Query)('query')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], GooglePlacesController.prototype, "search", null);
__decorate([
    (0, common_1.Get)('reverse-geocode'),
    (0, swagger_1.ApiOperation)({ summary: 'Reverse geocode coordinates' }),
    __param(0, (0, common_1.Query)('lat')),
    __param(1, (0, common_1.Query)('lng')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], GooglePlacesController.prototype, "reverseGeocode", null);
exports.GooglePlacesController = GooglePlacesController = __decorate([
    (0, swagger_1.ApiTags)('Google Places'),
    (0, common_1.Controller)('places'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, swagger_1.ApiBearerAuth)(),
    __metadata("design:paramtypes", [google_places_service_1.GooglePlacesService])
], GooglePlacesController);
//# sourceMappingURL=google-places.controller.js.map